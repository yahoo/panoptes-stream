...
WORKDIR /go/src/git.vzbuilders.com/marshadrad/panoptes/telemetry/simulator/
RUN CGO_ENABLED=0 go build . 

# run 
FROM alpine:latest 
COPY --from=builder /go/src/git.vzbuilders.com/marshadrad/panoptes/panoptes/panoptes /usr/bin/
COPY --from=builder /go/src/git.vzbuilders.com/marshadrad/panoptes/telemetry/simulator/simulator /usr/bin/
EXPOSE 8081
